...
extern void remote_rpc_with_mdm_nv_write(uint32 in_len, byte *input, uint32 *out_len, byte *output);	//CSFB SRD
extern void remote_rpc_with_mdm_nv_sync(uint32 in_len, byte *input, uint32 *out_len, byte *output);	//CSFB SRD
//extern const MmcPartition *lge_mmc_find_partition_by_name(const char *name);
...
//CSFB SRD
byte nv_backup_data[132];
boolean srd_userDataRecovery(void)
{
	const MmcPartition *pMisc_part;
	word status; 
	int max_counter , i=0;
	...
	nv_item_type remote_nvi;
	nv_item_info_type	nv_backuptype;
	boolean nv_recover_end = FALSE;
	byte result_mdm_efs_sync=0;
	uint32 out_len;

	char temp[6];

	printk(KERN_WARNING "[SRD]srd_userDataRecovery \n");

	//NV_LG_FW_SRD_DONE_I
	remote_nvi.item[0] = FALSE;  //initial value 
	
	memset(&nv_item_info,0x00,sizeof(xml_parser_nv_item_info));   //clear buffer
	nv_item_info.nv_enum = NV_LG_FW_SRD_DONE_I;
	remote_rpc_with_mdm(sizeof(nv_item_info), (byte *)&nv_item_info, &out_len, (byte *)&remote_nvi);
	 
	if(out_len==0)   //out len is zero means not active nv or fail 
	{
		printk(KERN_WARNING "[SRD]NV_LG_FW_SRD_DONE_I read -> Not Active\n");
	}
	else if(remote_nvi.item[0] == TRUE)
	{
		printk(KERN_WARNING "[SRD]NV_LG_FW_SRD_DONE_I read -> TRUE\n");
		return TRUE;
	}
	printk(KERN_WARNING "[SRD]NV_LG_FW_SRD_DONE_I length = %d , %02x %02x %02x %02x %02x %02x\n", (int)out_len, remote_nvi.item[0],remote_nvi.item[1],remote_nvi.item[2],remote_nvi.item[3],remote_nvi.item[4],remote_nvi.item[5]);
	...
	mtd_op_result=lge_read_block(srd_bytes_pos_in_emmc, (char*)&userDataBackUpInfo, sizeof(user_data_backup_info));//nb item num read
	if(mtd_op_result != sizeof(user_data_backup_info))
	{
		printk(KERN_ERR "[SRD]lge_read_block, error num = %d \n", mtd_op_result);
	}
	memcpy(temp, (char*)&userDataBackUpInfo, 6);
	printk(KERN_WARNING "[SRD]userDataBackUpInfo %02x %02x %02x %02x %02x %02x\n", temp[0], temp[1], temp[2], temp[3], temp[4], temp[5]);
	max_counter=userDataBackUpInfo.info.srd_backup_nv_counter ;
	srd_bytes_pos_in_emmc+=512;  //first sector is  info data

	if(max_counter ==0)
	{
		printk(KERN_ERR "[SRD]max_counter = %d \n", max_counter);
		return FALSE;
	}

	//while( ( i != max_counter ) && (nv_recover_end == FALSE))  ///dkdkdkdkddk
	while( ( i < max_counter ) ) 
	{
		memset(nv_item_buffer,0xff,256);
		mtd_op_result=lge_read_block(srd_bytes_pos_in_emmc, nv_item_buffer, 256);
		srd_bytes_pos_in_emmc += 256;
	    if(mtd_op_result != 256)
		{
	    	printk(KERN_ERR "[SRD]lge_read_block, error num = %d \n", mtd_op_result);
		}

		memcpy((void *)&nv_item_info,nv_item_buffer,sizeof(xml_parser_nv_item_info));

		nEnum = nv_item_info.nv_enum;
		nv_backuptype = nv_item_info.nv_type;
		i++;

		if(nEnum != NV_BACK_UP_LIST_END)
		{
			memcpy(nv_backup_data,(void *)&nEnum,4);
			memcpy(nv_backup_data+4,nv_item_buffer+sizeof(xml_parser_nv_item_info),128);

			   
			if (userDataBackUpInfo.info.backup_used == UPGRADE)	// Upgrade, Service Center
			{
				if((nv_backuptype == NV_UPGRADE) || (nv_backuptype == NV_BINDL) || (nv_backuptype == NV_REFURBISH))
				{
					printk(KERN_WARNING "[SRD]NV = %d \n", nEnum);
					remote_rpc_with_mdm_nv_write(sizeof(nv_backup_data), (byte *)&nv_backup_data, NULL, NULL);
				}	
			}
			else if (userDataBackUpInfo.info.backup_used == BINDL)	// Bindownload, Factory
			{
				if(nv_backuptype == NV_BINDL)
				{
					printk(KERN_WARNING "[SRD]NV = %d \n", nEnum);
					remote_rpc_with_mdm_nv_write(sizeof(nv_backup_data), (byte *)&nv_backup_data, NULL, NULL);
				}
			}
			else if (userDataBackUpInfo.info.backup_used == REFURBISH)	// Refurbish
			{
				if((nv_backuptype == NV_BINDL) || (nv_backuptype == NV_REFURBISH))
				{
					printk(KERN_WARNING "[SRD]NV = %d \n", nEnum);
					remote_rpc_with_mdm_nv_write(sizeof(nv_backup_data), (byte *)&nv_backup_data, NULL, NULL);
				}
			...
			else if (userDataBackUpInfo.info.backup_used == BOARDDL)	// Board Download
			{
				if(nEnum == NV_UE_IMEI_I)
				{
					status = 0;//NV_DONE_S;
				}
				else
			...
			else if (userDataBackUpInfo.info.backup_used == BOARDDLINHUNTSVILLE)	// Board Download in Huntsville
			{
				if(nEnum == NV_UE_IMEI_I)
				{
					status = 0;//NV_DONE_S;
	...
	nEnum = NV_LG_FW_SRD_DONE_I;
	memcpy(nv_backup_data,(void *)&nEnum,4);
	nv_backup_data[4] = TRUE;
	printk(KERN_WARNING "[SRD]NV = %d \n", nEnum);
	remote_rpc_with_mdm_nv_write(sizeof(nv_backup_data), (byte *)&nv_backup_data, NULL, NULL);

	// NV Sync
	remote_rpc_with_mdm_nv_sync(0, NULL, (uint32*)&out_len, (byte*)&result_mdm_efs_sync);
