...
void __init lge_make_fb_pmem(void)
{
	unsigned long ram_size = 0x398000; //1280*720*4
	unsigned long ram_base = 0x4a000000;
	int ret;

	ret = reserve_bootmem(ram_base, ram_size, BOOTMEM_EXCLUSIVE);
	if( ret < 0 ) {
		printk(KERN_INFO "frame copy buffer for hidden reset reservation failed - "
			"memory is in use(0x%lx) : 0x%08x\n", (unsigned long)ram_base, ret);
		return;
	}

	fb_copy_phys = ram_base;
	fb_copy_size = ram_size;

	printk("FB for HR start phys addr : %x\n", fb_copy_phys);
	printk("FB for HR start virt addr : %x\n", fb_copy_virt);
	printk("FB for HR size : %x\n", fb_copy_size);
}
#endif
...
void __init msm_fb_add_devices(void)
{
	...
	/* neo.kang@lge.com 2011-09-08, for hidden reset */
#if defined(CONFIG_LGE_HIDDEN_RESET_PATCH)
	fb_copy_virt = (size_t)ioremap(fb_copy_phys, fb_copy_size);
#endif
}
