...
static int lge_isa1200_hw_vib_on_off(struct lge_isa1200_context *context, bool on_off)
{
	...
	dev_dbg(context->dev.dev, "%s(%d)\n", __func__, on_off);

	if (on_off)	{
		//if (context->pdata->power) context->pdata->power(true);
		if (context->pdata->clock) context->pdata->clock(true);
...
#endif
		if (context->pdata->clock) context->pdata->clock(false);
...
static DEVICE_ATTR(vib_gain, S_IRUGO | S_IWUSR | S_IWGRP, lge_isa1200_vibrator_amp_show, lge_isa1200_vibrator_amp_store); 
...
*/
	...
	ret = device_create_file(context->dev.dev, &dev_attr_vib_gain);
...
static int __devexit lge_isa1200_remove(struct i2c_client *client)
{
	...
	device_remove_file(context->dev.dev, &dev_attr_vib_gain);
