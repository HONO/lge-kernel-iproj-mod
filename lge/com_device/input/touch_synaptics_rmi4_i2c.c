		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!ts->is_suspended) 
	{
		...
		{
			ret = i2c_smbus_write_byte_data(ts->client, DEVICE_CONTROL_REG,(DEVICE_CONTROL_REG_DOZE_LATER|DEVICE_CONTROL_REG_CONFIGURED));
			if (ret < 0){
				SYNAPTICS_ERR_MSG("DEVICE_CONTROL_REG write fail\n");
				return count;
			}
...
exit_work:
		...
		ret = i2c_smbus_write_byte_data(ts->client, DEVICE_CONTROL_REG,
				(DEVICE_CONTROL_REG_DOZE_LATER |DEVICE_CONTROL_REG_CONFIGURED));
		if (ret < 0)
			SYNAPTICS_ERR_MSG("DEVICE_CONTROL_REG write fail\n");
