...
int max17040_get_capacity_percent(void)
{
	...
	rc = max17040_read_data(max17040_i2c_client, MAX17040_SOC_MSB, &buf[0], 2);
	if(rc < 0)
		return pre_soc;
   ...
   if (batt_soc > 10000) batt_soc = 100;
    ...
    pr_info("[Fuel Gauge] FG:%d->%ld, FG:%d, MV:%d\n", (int)(debug_soc/10000000), debug_soc, (int)batt_soc, vbatt_mv);
#endif /* LGE_DEBUG_FINAL */
...
#else /* FEATURE_LGE_PM_FACTORY_FUEL_GAUGE */
